...
class QCreationInputVideo(QDialog):
    ...
    def create_video(self) -> None:
        ...
        if not(os.path.exists(path_ffmpeg) and os.path.exists(path_image)):
            return
        ...
        directory_image, image = os.path.split(path_image)
        ...
        # Transform video
        extension_video = "." +path_video.split(".")[-1]
        directory_video, video = os.path.split(path_video)
        image_without_extension = image.split(".")[0]
        ...
        path_output = self._path_output.text().replace("\\", "/")
